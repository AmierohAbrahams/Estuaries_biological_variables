...
Creating a theme for all the graphs used in this script: This theme was created and meet all the South African Journal of Marine Science visualisation requirements.
...
Now it is nessasary to load the data used in this assignment. This data comprise of *in situ* data collection as well as the coordinates needed for mapping the locations.
...
Map showing the Krom River: The Krom River is a tributary of the Doring River which forms part of the Olifants-Doorn River catchment in the Cederberg, Western Cape.
...
Table1: The averages of all of the physio_chemical variables measured in each site
--------------------------------------------------------------------------------------------------------------------------------------
...
Now we test to check the normality of our *in situ* collected data.
...
The non-parametric version of an ANOVA is a Kruskall-Wallis rank sum test: The correlation test was done to assess the impact of variables on biomass across both sites. As with the ANOVA, this first step with the Kruskall-Wallis test is not the last. We now run a post-hoc test on our results. For this a new package was installed called the pgirmess , which means we will need to load a new library pgirmess::kruskalmc().
...
kruskalmc(Temperature ~ Macrophyte_Biomass, data = R_assignment_AJ_)
kruskalmc(Depth ~ Macrophyte_Biomass, data = R_assignment_AJ_)
kruskalmc(pH ~ Macrophyte_Biomass, data = R_assignment_AJ_)
kruskalmc(Dissolved_oxygen ~ Macrophyte_Biomass, data = R_assignment_AJ_)
kruskalmc(TDS ~ Macrophyte_Biomass, data = R_assignment_AJ_)

```

When the data we want to compare are not continuous, but rather ordinal, we will need to use a Spearman test. Here we compare all the physiochemical parameters with macrophyte biomass.
...
Now we create a boxplot visualisation to compare the variation of each of the parameters for both of the sites. This further shows the variation that occurs between each of the sites.
...
Making a 3D scatter plot: Here we compare depth, temperature and macrophyte biomass across both of the sites.
...
Here we compare create a scatter plot to compare macrophyte biomass and depth. This was done to test whether or not macrophyte biomass is influenced by depth.
...
Studies employing a functional method with regards to community ecology typically direct their attention to the relationships exhibited between abiotic factors (physical and chemical parameters) and the composition and richness of macrophyte growth forms within freshwater ecosystems (e.g. Akasaka et al. 2010; Alahuta & Heino 2013). Hudon et al. 2000 suggest that the potential impacts of these environmental variables on the accumulation and variation in biomass amongst macrophytes, as basic as it may be, often goes under looked and is grossly underestimated. The application of these types of relationships have become more relevant in recent years, as the utility and value of these species as indicator species of both ecosystem function and health has become more apparent (Lacoul and Freedman, 2006). The main findings of this study reveal that the chemical and physical variables selected in this study, are incapable of explaining variation in the biomass of the submerged macrophyte *I.digitata*. Considering these findings, it is realized that our hypothesis that the biology of the submerged macrophyte *I.digitata* was directly correlated with physio-chemical attributes of its aquatic environment is subsequently rejected. The approaches employed in this study revealed the following: Firstly, despite significant differences in physio-chemical parameters (temperature, pH and TDS) between the selected upper and intermediate sites, no significant differences in the biomass of *I.digitata* were observed between the two sites. The findings of this study emphasize that the biomass accumulation of the submerged macrophyte *I.digitata*, is only slightly impacted by the measured physio-chemical parameters investigated in this study (reference the faceted graphs showing the correlations between biomass and the variables). Correlation tests revealed that biomass accumulation of I.digitata exhibits weak negative associations with both depth and temperature. A positive association exists between biomass accumulation and pH. Results indicate that chemical parameters such as total dissolved solids (TDS) and dissolved oxygen had little to no influence on biomass accumulation.

These findings of this study in terms of physio-chemical paramters on biomass accumulation come in direct contrast with the current body of information relating to the effects of physio-chemical variables on the variation of macrophyte life form biomass. Studies by Madsen et al. 2001 emphasize the impact of chemical parameters such as total dissolved solid (TDS) on the accumulation of biomass, as particle resuspension induced by water movement has the capacity to limit the penetration of light through the water column, and may serve as a limiting factor to photosynthetic activity in submerged macrophytes. Furthermore, studies by Junk and Piedade 1993, have demonstrated the effects of increased water depth on both net primary productivity and subsequent biomass accumulation- which they attribute to the horizontal expansion of macrophytes in deeper waters. In terms of pH, we find that studies conducted by Takahashi and Asaeda 2012, reveal that there is an increased rate of photosynthetic activity and subsequent biomass accumulation in waters that are slightly acidic. The rationale behind this statement lies in the fact that acidic water inevitably shifts the equilibrium balance of bicarbonate (HCO¬¬¬¬¬3¬) free CO2, which results in a higher amount of free CO2 ¬being made available for uptake by submerged macrophytes. Moreover, Takahashi and Asaeda (s=2012) have reported the role of temperature on submerged macrophyte biomass accumulation, which states that the physical attributes such as temperature have the capacity to affect chemical parameters such as dissolved oxygen, where higher water temperatures may lead to a decreased ability of gases to remain in solution (Takahashi & Asaeda 2012) 

Despite little to no influence of physio-chemical parameters on the accumulation of I.digitata biomass, our results indicate that physio-chemical parameters differed between the upper and intermediate sites along the Krom River, with significant differences observed with regards to physical parameters such as temperature and chemical parameters such as pH and total dissolved solids (TDS) (reference the boxplots facet graph). Physical parameters such as depth and chemical parameters such as dissolved oxygen appear to be more or less consistent between the two sites (reference the boxplot facet graph).
